...
(async()=>{
  const bytes=fs.readFileSync(path.join(root,'assets/expanded/island_expanded.glb'));
  const gltf=JSON.parse(bytes.toString('utf8',20,20+bytes.readUInt32LE(12)));
  const meta=JSON.parse(fs.readFileSync(path.join(root,'assets/expanded/scene.json')));
  ...
  fs.writeFileSync(path.join(root,'reports/expansion_gltf_validation.json'),JSON.stringify(report,null,2));
